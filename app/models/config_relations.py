...
class ParamGroup(db.Model, ToDictMixin):
    ...
    project_id = db.Column(db.Integer, nullable=True, comment='关联项目ID, NULL=全局（历史字段）')
    alg_type = db.Column(db.SmallInteger, default=2, comment='默认算法: 1=贝叶斯, 2=DOE枚举, 5=DOE文件')
    doe_file_name = db.Column(db.String(255), comment='DOE文件名')
    doe_file_heads = db.Column(db.JSON, comment='DOE文件表头')
    doe_file_data = db.Column(db.JSON, comment='DOE文件数据')
