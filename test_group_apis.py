...
def test_group_config_apis():
    ...
    # 1.1 获取参数组合列表
    param_groups = call_api("GET", "/param-groups", description="获取参数组合列表")
    
    # 1.2 创建参数组合
    new_param_group = call_api("POST", "/param-groups",
                               data={
                                   "name": "测试参数组合",
                                   "code": random_code("PG"),
    ...
    if new_param_group:
        ...
        # 1.3 更新参数组合
        call_api("PUT", f"/param-groups/{group_id}",
                data={"name": "测试参数组合（已更新）", "description": "更新后的描述"},
                description="更新参数组合")
        
        # 1.4 获取组合中的参数
        call_api("GET", f"/param-groups/{group_id}/params",
        ...
        if param_defs and len(param_defs) > 0:
            ...
            call_api("POST", f"/param-groups/{group_id}/params",
                    data={"paramDefId": param_id, "sort": 10},
                    description=f"添加参数 {param_id} 到组合")
            
            # 1.6 再次获取组合中的参数
            params_in_group = call_api("GET", f"/param-groups/{group_id}/params",
                                      description="获取组合中的参数（添加后）")
            
            # 1.7 从组合中移除参数
            if params_in_group and len(params_in_group) > 0:
                call_api("DELETE", f"/param-groups/{group_id}/params/{param_id}",
                        description=f"从组合中移除参数 {param_id}")
        
        # 1.8 删除参数组合
        call_api("DELETE", f"/param-groups/{group_id}",
                description="删除参数组合")
    ...
    # 2.1 获取工况输出组合列表
    cond_out_groups = call_api("GET", "/cond-out-groups", description="获取工况输出组合列表")
    
    # 2.2 创建工况输出组合
    new_cond_out_group = call_api("POST", "/cond-out-groups",
                                  data={
                                      "name": "测试工况输出组合",
    ...
    if new_cond_out_group:
        ...
        # 2.3 更新工况输出组合
        call_api("PUT", f"/cond-out-groups/{group_id}",
                data={"name": "测试工况输出组合（已更新）"},
                description="更新工况输出组合")
        
        # 2.4 获取组合中的工况
        call_api("GET", f"/cond-out-groups/{group_id}/conditions",
        ...
        if condition_defs and len(condition_defs) > 0:
            ...
            call_api("POST", f"/cond-out-groups/{group_id}/conditions",
                    data={"conditionDefId": condition_id, "sort": 10},
                    description=f"添加工况 {condition_id} 到组合")
            
            # 2.6 从组合中移除工况
            call_api("DELETE", f"/cond-out-groups/{group_id}/conditions/{condition_id}",
                    description=f"从组合中移除工况 {condition_id}")
        
        # 2.7 获取组合中的输出
        call_api("GET", f"/cond-out-groups/{group_id}/outputs",
        ...
        if output_defs and len(output_defs) > 0:
            ...
            call_api("POST", f"/cond-out-groups/{group_id}/outputs",
                    data={"outputDefId": output_id, "sort": 10},
                    description=f"添加输出 {output_id} 到组合")
            
            # 2.9 从组合中移除输出
            call_api("DELETE", f"/cond-out-groups/{group_id}/outputs/{output_id}",
                    description=f"从组合中移除输出 {output_id}")
        
        # 2.10 删除工况输出组合
        call_api("DELETE", f"/cond-out-groups/{group_id}",
