...
def test_cond_out_group_flow(client):
    ...
    add_output = client.post(
        f'/api/v1/config/output-groups/{group_id}/outputs',
        json={'outputDefId': output_id},
    )
    assert add_output.get_json()['code'] == ErrorCode.SUCCESS

    detail_resp = client.get(f'/api/v1/config/output-groups/{group_id}')
    assert detail_resp.get_json()['code'] == ErrorCode.SUCCESS

    remove_condition = client.delete(
        f'/api/v1/config/output-groups/{group_id}/conditions/{condition_id}'
    )
    assert remove_condition.get_json()['code'] == ErrorCode.SUCCESS

    remove_output = client.delete(
        f'/api/v1/config/output-groups/{group_id}/outputs/{output_id}'
    )
    assert remove_output.get_json()['code'] == ErrorCode.SUCCESS

    delete_resp = client.delete(f'/api/v1/config/output-groups/{group_id}')
    assert delete_resp.get_json()['code'] == ErrorCode.SUCCESS
